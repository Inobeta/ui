...
import { IbFilterBase, IbFilterDef, eq, none } from "@inobeta/ui";
...
The query is then passed to the remote data source as the `filter` property of its request object.
...
class UserDataSource extends IbTableRemoteDataSource<User, UserQuery> {
  ...
  fetchData(
    request: IbRemoteDataSourceRequest<UserQuery>
  ): Observable<IbFetchDataResponse<User>> {
    console.log(request.filter?.subscribed); // true or false
    // ...
  }
}
```
