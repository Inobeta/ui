...
<Meta title="Components/Forms (**Deprecated**)/Controls" />

# Controls

> ⚠️ Material Forms controls are deprecated. Follow the guide under **Forms (new)** for new forms.
...
> ⚠️ About `cols` and `rows` parameters: these will only work when `<ib-material-form [cols]="3" />` is set.
