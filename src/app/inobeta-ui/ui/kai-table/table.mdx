...
Working with a *flattened* dataset is always recommended to reduce complexity and save up development time spent
...
> 🚧 Until 17.1, nested data is not fully supported as it breaks filtering and sorting. These features do not take in consideration
> the `dataAccessor` function and will stop working in a use case similar to the one described.
