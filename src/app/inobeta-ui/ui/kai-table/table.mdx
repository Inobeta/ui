...
It will be used as a getter function in your template, and might be used during export as well with selected formats.
...
```html
<ib-text-column name="firstName" sort [sortingDataAccessor]="sortingDataAccessor"></ib-text-column>
```
...
export class IbCustomColumn<T> extends IbColumn<T> {
  ...
  sortingDataAccessor = (data: T, name: string): any => data[name].somethingNested;
}
```
...
```html
<ib-text-column name="firstName" sort [filterDataAccessor]="filterDataAccessor" />
```
...
export class IbCustomColumn<T> extends IbColumn<T> {
  ...
  filterDataAccessor = (data: T, name: string): any => data[name].somethingNested;
}
```
...
Server-side interactions are only available with an instance of `IbDataSource` used as input.
