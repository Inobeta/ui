...
View management is an optional feature provided by the decoupled `IbViewModule`. The table works without it — views are only needed when you want to let users save and restore table state (filters, sort, pagination).

To add views to a table, import `IbViewModule` alongside `IbKaiTableModule`, `IbTableActionModule`, and `IbFilterModule`.
