...
`tableName` is required and must be unique for each table on a page. It scopes the
NgRx state, URL state, views, selection events, and exported files.
...
`selectedView` is `null` when no view host is present, when no view is selected, or
when the implicit all-data view is active. Views are optional; the table does not
require `IbViewModule` or an `IbTableViewsHost`.
...
without it — views are only needed when you want to let users save and restore table state. Each view
captures a snapshot of the current filters, sort order, page size, and column aggregations.

### Setup

Import `IbViewModule` alongside `IbKaiTableModule` and `IbTableActionModule`. Filters are **not**
required: a view can be created and restored on any table, with or without an `ib-filter` component.
The only requirement is a unique `tableName` on `ib-kai-table`, which acts as the storage group key.
...
Click the **`+`** button next to the Default tab to create a view from the **current** table state.
The new view is appended to the list and automatically selected.
...
Use the context menu (`⋮`) on a named view tab and select **Rename**. Names are trimmed and must be
unique case-insensitively within the same table. Renaming does not re-apply the view or change the
table URL.
...
The selected view ID is stored in the table URL query parameter (`sv` field). On page load or browser
back/forward, the table resolves the view from `localStorage` by ID, within the correct `tableName`
group. If the URL contains an unknown or deleted view ID, the table falls back to Default.

### Limitations

- Old view data stored under the legacy `__redux-store-inobeta-ui__` hydration key is **not**
  migrated. DEVK-1065 starts from empty `localStorage`.
...
This is a **breaking change**: tables that previously relied on implicit
row-count-based sizing now fill their parent container by default.
See the [migration guide](#migration-guide) for how to adapt existing consumers.
...
Consumer code written before `tableHeight` was introduced must be updated.
The old behaviour auto-computed a `max-height` from `--ib-table-minimum-rows`
(default 10 rows) and Material row tokens. That logic is no longer active.
...
The following known consumers are in separate repositories and require
follow-up updates:

| Consumer | Action required |
| --- | --- |
| **Seed project** | Audit templates for `ib-kai-table` usage. Remove old CSS variable overrides. Choose `tableHeight` mode per table. |
| **Kaiboard** | Audit templates and adapters. Propagate `tableHeight` in any wrapper components. Remove old `max-height` workarounds. |
| **Simple Table adapter** | Expose a `tableHeight` input (or document the adapter's own default). Remove old sizing logic based on the removed CSS variables. |

> These repositories are not part of this codebase. Perform the listed actions
> in each repository before releasing this library version.
...
to be implemented. The request is a value object containing `sort`, `pageIndex`, `pageSize`, and `filter`.
...
The URL stores the full canonical state for each table, including explicit `null`
values. Sort and filter changes reset the page to zero. Browser back/forward
replays the URL snapshot, restoring sort, filters, view, pagination, and aggregation.
...
```typescript
import { ibTableSelectLastQueryString } from "public_api";
