...
| 4 | Technical default (`sort: null`, `filters: null`, `selectedView: null`, page `0`, page size `20`) |
...
`filters: null` clears filters even when a view or `tableDef` defines them.
...
Use signals directly when reading table state:

```typescript
urlState = this.store.selectSignal(ibTableSelectUrlState("YOUR_TABLE_NAME"));
lastQueryString = this.store.selectSignal(
