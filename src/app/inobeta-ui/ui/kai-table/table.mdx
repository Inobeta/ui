...
<ib-kai-table [data]="data" [displayedColumns]="displayedColumns">
  ...
  ></ib-text-column>
  <ib-text-column
    [headerText]="'user.surname' | translate"
    name="surname"
  ></ib-text-column>
</ib-kai-table>
```
...
```html
<ib-number-column name="price" aggregate></ib-number-column>
```
...
.ib-table__container {
  ...
  --ib-table-content-max-height: calc(56px + 520px);
...
Wrapper element that contains the table itself alongside views, actions, filters, and paginator.
Use this class in your queries in order to override variables and sections of this component.

- `.ib-table__toolbar`
- `.ib-table__content`
- `.ib-table__row`
- `.ib-table__row .mat-mdc-cell`
...
```css
.ib-table__container {
  .ib-table__row:nth-child(odd) {
    background-color: lightgreen;
  }

  .ib-table__row:nth-child(even) {
...
Enable server-side interactions, such as data fetching, pagination, sorting, and filtering with an `IbTableRemoteDataSource` provided to the `dataSource` input.

1. Define your data source

Your data source must extend `IbTableRemoteDataSource` in order to be compatible with the table. This is an abstract class, as it needs the `fetchData` method
to be implemented. 

The method `fetchData` will be called with three parameters whenever some data from the server is needed: `sort` is of type `MatSort`¹,
the current sorted column is available in `sort.active` and the order in `sort.direction`².
Then, `page` is of type `MatPaginator`³, the requested page index can be accessed with `page.pageIndex`, and the current page size with `page.pageSize`.
...
> Remember: You must instance your data source inside your component's constructor (like the example above), and that you can inject 
