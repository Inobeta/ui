...
> **Deprecated:** `IbHttpModule` is deprecated and scheduled for removal in v21. This
> page documents its configuration API for existing consumers.
...
Legacy action: only stores a token in storage. It does not call the `login()` function.
