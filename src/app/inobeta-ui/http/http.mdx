...
> **Deprecated:** `IbHttpModule` is deprecated and scheduled for removal in v21. Keep
> this page for existing integrations; symbols with their own deprecation metadata
> are marked below.

## Setup

This module provides an application-wide store and must be registered with the features it uses.

- Register `IbHttpModule` with the application providers.
- Add `provideState` for `ibSessionFeature` and `ibLoaderFeature`.
- Add the effects provided in `ibHttpEffects`.
...
- Authentication interceptor
  - Sets the `Authorization` header for authenticated HTTP requests
- Error interceptor
...
> **Deprecated:** `IbLoadingDirective` and its skeleton components are deprecated and
> scheduled for removal in v21.
...
> **Deprecated:** `IbRoleCheckDirective` is deprecated and scheduled for removal in v21.
...
> **Deprecated:** `IbSpinnerLoadingComponent` is deprecated and scheduled for removal in v21.
...
> **Deprecated:** `IbAuthGuard` is deprecated and scheduled for removal in v21.
...
> **Deprecated:** `ibCrudToast` is deprecated and scheduled for removal in v21.
